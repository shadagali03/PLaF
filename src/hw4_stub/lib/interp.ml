...
and
  ...
  (* derefercnes the value that is given *)
  | Proj(e,id) ->
    eval_expr e >>= fun expression ->
      fields_of_recordVal expression >>= fun l ->
  ...
  | IsNumber(e) ->
    eval_expr e >>=
      ...
      | NumVal _ -> return @@ BoolVal (1 > 0) (*Idk if its true or True*)
      | _ -> return @@ BoolVal (1 < 0)) *)
  | Unit -> return UnitVal
  | Debug(_e) ->
